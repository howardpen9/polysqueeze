...
impl FillProcessor {
    ...
    /// Finalize an order and move its pending fills to processed
    pub fn finalize_order(&mut self, order_id: &str) {
        if let Some(fills) = self.pending_fills.remove(order_id) {
            self.processed_fills.extend(fills);
        }
    }

    /// Handle a streaming message and forward any fill events
    pub fn handle_stream_message(&mut self, message: &StreamMessage) -> Result<()> {
        match message {
            StreamMessage::Trade { data }
            | StreamMessage::UserTrade { data }
            | StreamMessage::MarketTrade { data } => self.process_fill(data.clone()),
            _ => Ok(()),
        }
    }
    ...
    /// Get total pending volume for an order ID
    pub fn pending_volume_for(&self, order_id: &str) -> Decimal {
        self.pending_fills
            .get(order_id)
            .map(|fills| fills.iter().map(|f| f.size).sum())
            .unwrap_or(Decimal::ZERO)
    }

    /// Get the total pending volume across all orders
    pub fn total_pending_volume(&self) -> Decimal {
        self.pending_fills.values().flatten().map(|f| f.size).sum()
    }

    /// Get the total processed volume across all orders
    pub fn total_processed_volume(&self) -> Decimal {
        self.processed_fills.iter().map(|f| f.size).sum()
    }
...
mod tests {
    ...
    fn test_fill_processor_basic_operations() {
        ...
        let result = processor.process_fill(fill_event.clone());
        assert!(result.is_ok());

        // Check that the fill was added to pending
        assert_eq!(processor.pending_fills.len(), 1);
        assert_eq!(processor.pending_volume_for("order_1"), dec!(25));

        processor.finalize_order("order_1");
        assert_eq!(processor.pending_fills.len(), 0);
        assert_eq!(processor.total_processed_volume(), dec!(25));
    }

    #[test]
    fn test_fill_processor_stream_message() {
        let mut processor = FillProcessor::new(10);
        let fill_event = FillEvent {
            id: "stream_fill".to_string(),
            order_id: "stream_order".to_string(),
            side: Side::SELL,
            size: dec!(150),
            price: dec!(0.65),
            timestamp: chrono::Utc::now(),
            token_id: "token_stream".to_string(),
            maker_address: alloy_primitives::Address::ZERO,
            taker_address: alloy_primitives::Address::ZERO,
            fee: dec!(0.02),
        };

        let stream_msg = StreamMessage::Trade {
            data: fill_event.clone(),
        };
        assert!(processor.handle_stream_message(&stream_msg).is_ok());
        assert_eq!(processor.pending_volume_for("stream_order"), dec!(150));
    }
}
