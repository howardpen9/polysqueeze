...
pub mod time {
    ...
    pub fn secs_to_datetime(timestamp: u64) -> DateTime<Utc> {
        DateTime::from_timestamp(timestamp as i64, 0).unwrap_or(Utc::now())
    }
}
...
pub mod crypto {
        ...
        T: ?Sized + Serialize,
    {
        let decoded = URL_SAFE
            .decode(secret)
            .map_err(|e| PolyError::config(format!("Invalid secret format: {}", e)))?;
        ...
        let mut mac = HmacSha256::new_from_slice(&decoded)
            .map_err(|e| PolyError::internal("HMAC initialization failed", e))?;
...
pub mod retry {
        ...
        Fut: Future<Output = Result<T>>,
    {
        ...
        Err(last_error.unwrap_or_else(|| {
            PolyError::internal(
                "Retry loop failed",
                std::io::Error::new(std::io::ErrorKind::Other, "No error captured"),
            )
        }))
    }
}
...
pub mod address {
    ...
    pub fn parse_address(addr: &str) -> Result<Address> {
        Address::from_str(addr)
            .map_err(|e| PolyError::validation(format!("Invalid address format: {}", e)))
    }

    /// Validate token ID format
    pub fn validate_token_id(token_id: &str) -> Result<()> {
        if token_id.is_empty() {
            return Err(PolyError::validation("Token ID cannot be empty"));
        }

        // Token IDs should be numeric strings
        if !token_id.chars().all(|c| c.is_ascii_digit()) {
            return Err(PolyError::validation("Token ID must be numeric"));
        }
    ...
    pub fn token_id_to_u256(token_id: &str) -> Result<U256> {
        ...
        U256::from_str_radix(token_id, 10)
            .map_err(|e| PolyError::validation(format!("Invalid token ID: {}", e)))
    }
}
