...
impl PolyError {
    ...
    pub fn is_retryable(&self) -> bool {
        match self {
            PolyError::Network { .. } => true,
            PolyError::Api { status, .. } => {
                // 5xx errors are typically retryable
                *status >= 500 && *status < 600
            }
            PolyError::Timeout { .. } => true,
            PolyError::RateLimit { .. } => true,
            PolyError::Stream { kind, .. } => {
                matches!(
                    kind,
    ...
    pub fn retry_delay(&self) -> Option<Duration> {
        match self {
            PolyError::Network { .. } => Some(Duration::from_millis(100)),
            PolyError::Api { status, .. } => {
                if *status >= 500 {
            ...
            PolyError::Timeout { .. } => Some(Duration::from_millis(50)),
            PolyError::RateLimit { retry_after, .. } => {
                retry_after.or(Some(Duration::from_secs(1)))
            }
            PolyError::Stream { .. } => Some(Duration::from_millis(250)),
            _ => None,
        }
    }
    ...
    pub fn is_critical(&self) -> bool {
        match self {
            PolyError::Auth { .. } => true,
            PolyError::Config { .. } => true,
            PolyError::Internal { .. } => true,
            PolyError::Order { kind, .. } => {
    ...
    pub fn category(&self) -> &'static str {
        match self {
            PolyError::Network { .. } => "network",
            PolyError::Api { .. } => "api",
            PolyError::Auth { .. } => "auth",
            PolyError::Order { .. } => "order",
            PolyError::MarketData { .. } => "market_data",
            PolyError::Config { .. } => "config",
            PolyError::Parse { .. } => "parse",
            PolyError::Timeout { .. } => "timeout",
            PolyError::RateLimit { .. } => "rate_limit",
            PolyError::Stream { .. } => "stream",
            PolyError::Validation { .. } => "validation",
            PolyError::Internal { .. } => "internal",
        }
    }
}
...
impl From<reqwest::Error> for PolyError {
    fn from(err: reqwest::Error) -> Self {
        if err.is_timeout() {
            ...
            PolyError::network("HTTP request failed", err)
        } else {
            PolyError::internal("Unexpected reqwest error", err)
        }
    }
}
...
impl From<url::ParseError> for PolyError {
    fn from(err: url::ParseError) -> Self {
        PolyError::config(format!("Invalid URL: {}", err))
    }
}

impl From<tokio_tungstenite::tungstenite::Error> for PolyError {
    fn from(err: tokio_tungstenite::tungstenite::Error) -> Self {
        ...
        PolyError::stream(format!("WebSocket error: {}", err), kind)
    }
}

// Manual Clone implementation since Box<dyn Error> doesn't implement Clone
impl Clone for PolyError {
    fn clone(&self) -> Self {
        match self {
            PolyError::Network { message, source: _ } => PolyError::Network {
            ...
            PolyError::Auth { message, kind } => PolyError::Auth {
                message: message.clone(),
                kind: kind.clone(),
            },
            PolyError::Order { message, kind } => PolyError::Order {
                message: message.clone(),
                kind: kind.clone(),
            },
            PolyError::MarketData { message, kind } => PolyError::MarketData {
                message: message.clone(),
                kind: kind.clone(),
            },
            PolyError::Config { message } => PolyError::Config {
                message: message.clone(),
            },
            PolyError::Parse { message, source: _ } => PolyError::Parse {
                message: message.clone(),
                source: None,
            },
            PolyError::Timeout {
            ...
            PolyError::Stream { message, kind } => PolyError::Stream {
                message: message.clone(),
                kind: kind.clone(),
            },
            PolyError::Validation { message, field } => PolyError::Validation {
                message: message.clone(),
                field: field.clone(),
            },
            PolyError::Internal { message, source: _ } => PolyError::Internal {
                message: message.clone(),
                source: None,
            },
        }
